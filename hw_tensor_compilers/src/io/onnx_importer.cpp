...
namespace io {
    ...
    auto map_dtype(int32_t onnx_type) -> DataType {
        switch (onnx_type) {
    ...
    auto map_operator_type(const std::string& op) -> OperatorType {
    ...
    auto get_weights(const onnx::TensorProto& tensor) -> AttributeValue {
    ...
    auto parse_attribute(const onnx::AttributeProto& attr) -> AttributeValue {
        switch (attr.type()) {
    ...
    auto convert_value_info(const onnx::ValueInfoProto& info, Graph& g) -> ValueID {
        Shape shape;
    ...
    auto OnnxImporter::import_graph() -> graph_engine::Graph {
        ...
        Graph graph;
        const onnx::GraphProto& onnx_graph = protobuf_model.graph();
        ...
        graph.reserve(
            protobuf_model.graph().node_size(),
            protobuf_model.graph().initializer_size()
        );
        graph.inputs.reserve(protobuf_model.graph().input_size());
        graph.outputs.reserve(protobuf_model.graph().output_size());
